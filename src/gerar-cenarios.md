...
inputs:
  - name: formato
    required: true
    description: "Formato de saída (único suportado: gherkin)"
  - name: modulo
    required: false
    description: "Gera cenários apenas para o módulo especificado"
  - name: requisitos
    required: false
    description: "Caminho para documento de requisitos (PDF, DOCX, Markdown, TXT). Ativa modo requirements-driven — gera cenários a partir dos requisitos em vez de (ou além de) artefatos de exploração"
targets:
  claude:
    supported: true
  opencode:
