---
id: executar-planilha
name: "Execução em Lote por Planilha"
type: script
language: pt-BR
provider_hint: cheap
tools_required:
...
inputs:
  - name: arquivo
    required: true
    description: "Caminho da planilha de cenários (.xlsx ou .csv)"
  - name: modulo
    required: false
    description: "Executa apenas cenários do módulo especificado"
  - name: prioridade
    required: false
  ...
  - name: max-falhas
    required: false
    description: "Falhas consecutivas para ativar circuit breaker (padrão: 10)"
  - name: retry-falhos
    required: false
    description: "Re-executar cenários falhos ao final (flag booleano)"
targets:
  claude:
    supported: true
  opencode:
